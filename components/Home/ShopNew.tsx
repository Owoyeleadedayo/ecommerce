...
const products = [
  ...
  { id: 103, name: "Iron Pan", price: 7000, image: "/images/ironPan3.png" },
];

const mapToProduct = (item: (typeof products)[0]): Product => ({
  id: `utensil-${item.id}`,
...
const ShopCard = ({ product, onAddToCart }: ShopCardProps) => {
  return (
    <Card className="group relative w-full md:w-75 py-0 overflow-hidden">
      {/* Background-image: works on Safari, Opera Mini, all browsers */}
      <div
        style={{
...
const ShopNew = () => {
  ...
  return (
    <section className="flex flex-col bg-white justify-center items-center py-6 md:py-16 px-6 md:px-16 lg:px-45 gap-10">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 place-items-center gap-8">
        {products.map((product) => (
          <ShopCard
            key={product.id}
            product={product}
            onAddToCart={() => dispatch(addToCart(mapToProduct(product)))}
          />
        ))}
      </div>
    </section>
